...
fn main() {
    ...
    if let Some(path) = find_path(grid, start, goal) {
        ...
        for step in &path {
            println!("Langkah ke: ({}, {})", step.0, step.1);
        }
        println!("Jumlah langkah: {}", path.len() - 1);
    } else {
...
fn find_path(grid: Vec<Vec<i32>>, start: (usize, usize), goal: (usize, usize)) -> Option<Vec<(usize, usize)>> {
    let rows = grid.len(); // Jumlah baris dalam peta.
    let cols = grid[0].len(); // Jumlah kolom dalam peta.
    ...
    // Vektor untuk merepresentasikan 4 arah gerakan (atas, bawah, kiri, kanan).
